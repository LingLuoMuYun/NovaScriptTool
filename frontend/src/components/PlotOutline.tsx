    ...
    <div className="space-y-6">
      ...
      {outline && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-gray-800">📖 故事大纲</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ...
              <div
                key={item.label}
                className={`rounded-lg border-l-4 ${item.color} bg-gray-50 p-4`}
              >
      ...
      {timeline && timeline.length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-gray-800">⏱️ 时间线</h2>
            ...
            {timeline.map((item, i) => (
              <div key={i} className="flex gap-4">
                <div className="flex flex-col items-center">
                  ...
                  {i < timeline.length - 1 && (
                    <div className="mt-1 h-full w-0.5 bg-indigo-100" />
                  )}
                </div>
                <div className="pb-4">
                  <p className="text-sm font-medium text-gray-800">{item.event}</p>
                  {item.chapter && (
                    <p className="mt-0.5 text-xs text-gray-400">{item.chapter}</p>
                  )}
                </div>
              </div>
            ))}
      ...
      {conflicts && conflicts.length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-gray-800">⚡ 剧情冲突</h2>
          <div className="space-y-3">
            {conflicts.map((c, i) => (
              <div key={i} className="rounded-lg bg-red-50 p-4">
