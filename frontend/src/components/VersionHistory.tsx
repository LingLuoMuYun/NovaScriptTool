  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      {diffMode ? (
        ...
        <p className="p-6 text-center text-sm text-gray-400">暂无版本记录</p>
      ) : (
        <div className="max-h-96 overflow-y-auto">
          {/* Diff 选择器 */}
          <div className="border-b border-gray-100 bg-gray-50 px-4 py-2">
            <p className="mb-2 text-xs text-gray-500">选择两个版本对比差异：</p>
            ...
            {versions.map((v, i) => (
              <div key={v.id} className="relative flex gap-3 pb-4">
                ...
                <div className={`z-10 mt-0.5 h-3 w-3 rounded-full border-2 ${
                  i === 0 ? "border-indigo-500 bg-indigo-100" : "border-gray-300 bg-white"
                }`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-gray-700">v{v.version}</span>
                    <span className={`rounded px-1.5 py-0.5 text-xs ${
                      v.createdBy === "user" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-500"
                    }`}>
                  ...
                  <button
                    ...
                    className="mt-1 text-xs text-indigo-500 hover:text-indigo-700 disabled:text-gray-300"
                  >
